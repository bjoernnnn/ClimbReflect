...
### Schritt 3 — Leckquelle isolieren (jetzt der aktive Schritt)
**Ziel:** Den dominierenden Verbraucher der ~300 MB finden.
**Aufgabe (passt zum Memory-Log-Workflow):** Diagnose-Build mit **reduzierter HealthKit-
Collection** – `HKWorkoutSession` (für Hintergrund) behalten, aber `HKLiveWorkoutBuilder`/
`beginCollection` testweise weglassen oder nur HF sammeln. Memory-Log beobachten.
**Erwartete Auswertung:**
- Grundleck flach (kein Anstieg im Schlaf/Idle) → **`HKLiveWorkoutBuilder`-Akkumulation** ist die
  (Grund-)Quelle.
- Klettert weiter / nur bei Interaktion → **Rendering/SwiftUI** ist der (schnelle) Verursacher
  → Allocations-/Memory-Graph-Profiler.
**Hinweis:** Für den realen Kletteralltag (schnelle Abbrüche) ist der rendering-/
interaktionsabhängige Anteil der Hauptverursacher – ggf. dort zuerst ansetzen.
**Status:** _offen (nächster Schritt)_
